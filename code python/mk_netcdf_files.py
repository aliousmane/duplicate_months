    ...
    for st,station in enumerate(StationIDs):
        ...
        for rfile in raw_files:
            ...
            if rfile_year in ValidYears:
                ...
                try:
                    with gzip.open(rfile,'rt') as infile:
...
#        nc_var.standard_name = "station_identification_code"
        ...
        #print(StationIDs[st])#
        #stationIDst=np.array(ncdf.stringtoarr(StationIDs[st],12))
        #print(stationIDst.shape)
        #print(nc_var.shape)
        #nc_var[:] = ncdf.stringtochar(stationIDst)
        nc_var[:] = ncdf.stringtoarr(StationIDs[st],12)
        print("nc_var[:]")#
        print(nc_var[:])#
        # create variables
        ...
        if hours:
            timesvar.units='hours since {}'.format(dt.datetime.strftime(dt.datetime(STARTYEAR,1,1,0,0), "%Y-%m-%d"))
        else:
            timesvar.units='days since {}'.format(dt.datetime.strftime(dt.datetime(STARTYEAR,1,1,0,0), "%Y-%m-%d"))
        timesvar.axis='T'
...
#        stationsvar.units='USAF - WBAN from ISD source'
        ...
        WriteAtt ributes(tempsvar,'Dry bulb air temperature at screen height (~2m)','latitude: longitude: time: point (nearest to reporting hour)',FLTMDI,'degree_Celsius','T',tmin,tmax,'latitude longitude elevation',standard_name = 'surface_temperature')
        ...
        WriteAttributes(ppt1pvar,'Reported period over which precipitation was recorded','latitude: longitude: point', INTMDI, 'hour', 'T', 0, 98,'latitude longitude elevation precip1_depth') #, standard_name = 'period_of_precipitation_report')
        ...
        if Extra:
            ...
            WriteAttributes(ppt2pvar,'Reported period over which precipitation was recorded','(as ISD variable precip2)', INTMDI, 'hour', 'T', 0, 98,'latitude longitude elevation precip2_depth')
            ...
            WriteAttributes(ppt3pvar,'Reported period over which precipitation was recorded','(as ISD variable precip3)', INTMDI, 'hour', 'T', 0, 98,'latitude longitude elevation precip3_depth')
            ...
            WriteAttributes(ppt4pvar,'Reported period over which precipitation was recorded','(as ISD variable precip4)', INTMDI, 'hour', 'T', 0, 98,'latitude longitude elevation precip4_depth')
...
#        idsvar[:]=StationIDs[st]
        print(input_station_id.shape)
        print(stationsvar[:].shape)
        stationsvar[:,0]=ncdf.stringtochar(input_station_id)
        ...
        ppt1cvar[:,0]=ncdf.stringtochar(precip1_condition)
