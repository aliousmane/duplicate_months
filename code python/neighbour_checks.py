...
def neighbour_checks(station_info, restart_id = "", end_id = "", distances=np.array([]), angles=np.array([]), second = False, masking = False, doZip=False, plots = False, diagnostics = False):
    ...
    for st, stat in enumerate(station_info):       
        ...
        clean_up.clu(station, ["temperatures","dewpoints","slp","windspeeds","winddirs"], [44,45,46,47,48], FLAG_COL_DICT, DATASTART, DATAEND, logfile, plots = plots, diagnostics = diagnostics)
...
if __name__=="__main__":
    ...
    parser.add_argument('--masking', dest='masking', action='store_true', default = True,
                        help='Apply flags')
