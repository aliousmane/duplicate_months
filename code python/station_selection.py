#!/D/Logiciel/Anaconda3/Library\usr/bin/python3.6.1
#------------------------------------------------------------
#                    SVN Info
...
ISD_DOWNLOAD = "connexion_ftp.txt"
ISD_LISTING = "isd-history.csv"
ISD_INVENTORY = "isd-inventory.txt"
INPUT_FILE_LOCS
INPUT_FILE_LOC="hadobs2/hadisd/v201_2016p/code_v201_2016p/input_files/"
# what is available
START_YEAR = 1901
    ...
    try:
        with open(INPUT_FILE_LOCS+ISD_LISTING, 'r') as infile:
            ...
            while(iter<2512) :
                ...
                iter=iter+1
            while(iter<2562) :
                for line  in infile  :  #Limiter taille des données lues
                    iter=iter+1
                    lc += 1
                    if lc <=1 : continue
                    fields = line.split(",")
                    ...
                    lon = fields[7].strip()
                    elev = fields[8].strip()
                    start = dt.datetime.strptime(fields[9],"%Y%m%d").date()
...
def process_inventory(candidate_stations, data_start, data_end, min_years_present, do_mergers = True):
    ...
    try:
        inventory = np.genfromtxt(INPUT_FILE_LOCS+ISD_INVENTORY, skip_header = 603000, dtype = int)
    except OSError:
        pass
    final_stations = []
...
def process_germany(all_stations):
    ...
    for gid in german_ids:
        ...
        if len(old_id_loc[0]) != 0:
            ...
            old_stn = all_stations[old_id_loc[0][0]]
            ...
            if np.product(probs) > PROB_THRESHOLD:
                ...
                all_stations[old_id_loc[0][0]].start = dt.datetime.today()
                all_stations[old_id_loc[0][0]].end = dt.datetime.today()
...
def doftp_transfer(infile,diagnostics = False):
    ...
    if diagnostics: 
        print ("getting file")
        os.system('ftp -s:{:s}'.format(infile))
        print ("got files")
...
def find_matches(candidate_stations):
    ...
    outfile = open(INPUT_FILE_LOCS+'internal_merging_log.txt','w')
...
def external_merges(candidate_stations, all_stations, data_start, data_end):
    ...
    outfile = open(INPUT_FILE_LOCS+'external_merging_log.txt','w')
    outfile.write("Station1, Station2, Probs, Prob-product\n\n")
    ...
    for l,locs in enumerate(match):
        if locs != []:
            ...
            # np.maximum returns the max of each array, element wise
            monthly_obs=[]
            monthly_obs += [extract_inventory(all_stations[loc], inventory, data_start, data_end, do_mergers = False) for loc in locs]
...
#*********************************************
def run_selection(data_start = 2015, 
    ...
    if updateFiles:
        doftp_transfer(ISD_DOWNLOAD)


    # parse text file into candidate list, with lat, lon, elev and time span limits applied
    all_stations, candidate_stations = read_stations(min_years_present, start_year = data_start, uk = uk, canada = False, germany = True)      

    if plots: plot_stations(candidate_stations, 'isd_lat_lon_elev_{}.png'.format(HADISD_VERSION))
    # write candidate files
    outfile = open(INPUT_FILE_LOC+"candidate_stations_lat_lon_elev.txt",'w')
    for stn in candidate_stations:
    ...
    # write candidate files
    outfile = open(INPUT_FILE_LOC+"candidate_stations_6hourly.txt",'w')
    for stn in candidate_stations:
        outfile.write("{:12s} {:7.3f} {:8.3f} {:7.1f}\n".format(stn.id, stn.lat, stn.lon, stn.elev))
    outfile.close()
    ...
    outfile = open(INPUT_FILE_LOC+"internal_mergers.txt", 'w')
    for merges in ordered_merges:
    ...
    outfile = open(INPUT_FILE_LOC+"final_mergers.txt", 'w')                
    for stn in candidate_stations:
        if stn.mergers != []:
    ...
    # write candidate files
    outfile = open(INPUT_FILE_LOC+"candidate_stations.txt",'w')
    for stn in candidate_stations:
        outfile.write("{:12s} {:7.3f} {:8.3f} {:7.1f}\n".format(stn.id, stn.lat, stn.lon, stn.elev))
    outfile.close()

    # write output files - full details
    outfile = open(INPUT_FILE_LOC+"candidate_stations_details.txt",'w')
    for stn in candidate_stations:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--start', dest='start', action='store', default = 2016,
    ...
    parser.add_argument('--years_present', dest='yearsPresent', action='store', default = 1,
