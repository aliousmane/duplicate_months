        ...
        for month in range(12):
            ...
            if variable in ["slp", "windspeeds"]:
                ...
                for y, year in enumerate(range(month_ranges.shape[0])):
                    ...
                    if y == 0:
                        winds_year = winds.data[int(month_ranges[year,month][0]):int(month_ranges[year,month][1])]
                        winds_month = winds_year.reshape(-1,24)
                                            
                        slp_year = slp.data[int(month_ranges[year,month][0]):int(month_ranges[year,month][1])]
                    ...
                    if variable in ["slp", "windspeeds"]:
                        ...
                        if np.abs((variances[y] - median_variance) / iqr_variance) > iqr_threshold:
                            ...
                            if (biggest_neg < 10) and (biggest_pos < 10) and not storm:
                                ...
                                station.qc_flags[int(month_ranges[year,month][0]):int(month_ranges[year,month][1]), flag_col[v]] = 1
                                if plots or diagnostics:
                            ...
                            if plots:
                                # plot showing the pressure, pressure first differences and the wind speeds
                                plot_times = utils.times_hours_to_datetime(station.time.data[int(month_ranges[year,month][0]):int(month_ranges[year,month][1])], start)
