...
def cu_plots(times, indata, start, end, datastart, title, extra_text = ""):
    ...
    YLABELS = {"temperatures":"Temperature (C)", "dewpoints":"Dewpoints (C)", "slp":"SLP (hPa)", "windspeeds":"Wind Speed (m/s)","winddirs":"Wind Direction"}
...
def clean_up(st_var, flags, input_flag_cols, out_flag_col, start, end, times, plots = False):
    ...
    for month in month_ranges:
        ...
        this_month = filtered[int(month[0]):int(month[1])]
        ...
        # if 40% of obs flagged, then flag remainig
            ...
            flag_locs = np.where(total_flags[int(month[0]):int(month[1])] > 0)[0]
