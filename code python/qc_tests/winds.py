...
def wind_rose_check(station, flag_col, start, end, logfile, plots = False, diagnostics = False):
    ...
    for y,year in enumerate(month_ranges_years):
        ...
        if len(direction[int(year[0][0]):int(year[-1][0])].compressed()) > 0:

            hist, binEdges = np.histogram(direction[int(year[0][0]):int(year[-1][0])],  bins = bins, normed = True)
    ...
    #    plot_gaussian = utils.funcGH(fit[0], bincenters)
        ...
        for y,year in enumerate(month_ranges_years):
                ...
                if rmse[y] > gap:
                    ...
                    good, = np.where(np.logical_or(direction.mask[int(year[0][0]):int(year[-1][0])] == False, speed.mask[int(year[0][0]):int(year[-1][0])] == False))
