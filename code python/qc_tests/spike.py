    ...
    for v, variable in enumerate(variable_list):
        ...
        for month in range(12):
            for year in range(month_ranges.shape[0]):
                ...
                if year == 0:
                    this_month_time_diff = full_time_diffs[int(month_ranges[year,month,0]):int(month_ranges[int(year),month,1])]
                    this_month_filtered_diff = full_filtered_diffs[int(month_ranges[year,month,0]):int(month_ranges[year,month,1])]
                else:
                    this_month_time_diff = np.ma.concatenate([this_month_time_diff, full_time_diffs[int(month_ranges[year,month,0]):int(month_ranges[year,month,1])]])
                    this_month_filtered_diff = np.ma.concatenate([this_month_filtered_diff, full_filtered_diffs[int(month_ranges[year,month,0]):int(month_ranges[year,month,1])]])


                month_locs[int(month_ranges[year,month,0]):int(month_ranges[year,month,1])] = month
        ...
        for t in np.arange(len(time_diffs)):
            if (np.abs(time_diffs[t - 1]) > 240) and (np.abs(time_diffs[t]) < 3):
                ...
                if (critical_values[int(time_diffs[t]) - 1, int(month_locs[t])] != st_var.mdi):
                    
                    # jump from spike > critical but average after < critical / 2
                    if (np.abs(median_diff) < critical_values[int(time_diffs[t]) - 1, int(month_locs[t])] / 2.) and\
                        (np.abs(next_diff) > critical_values[int(time_diffs[t]) - 1, int(month_locs[t])]) :
                ...
                if (critical_values[int(time_diffs[t - 1]) - 1, int(month_locs[t])] != st_var.mdi):
                    
                    # jump into spike > critical but average before < critical / 2
                    if (np.abs(median_diff) < critical_values[int(time_diffs[t - 1]) - 1, int(month_locs[t])] / 2.) and\
                        (np.abs(prev_diff) > critical_values[int(time_diffs[t - 1]) - 1, int(month_locs[t])]) :
            ...
            for spk_len in [1,2,3]:
                if t >= spk_len and t < len(time_diffs) - spk_len:
                    ...
                    ((spk_len == 3) and (np.abs(time_diffs[t - spk_len + 1]) <= spk_len * 3) and (np.abs(time_diffs[t - spk_len + 2]) <= spk_len * 3))):
                        ...
                        (critical_values[int(time_diffs[t - spk_len]) - 1, int(month_locs[t])] != st_var.mdi):
                        
                            # if exceed critical values
                            if (np.abs(value_diffs[t - spk_len]) >= critical_values[int(time_diffs[t - spk_len] )- 1, int(month_locs[t])]):
                                ...
                                if (math.copysign(1, value_diffs[t]) != math.copysign(1, value_diffs[t - spk_len])):
                                    ...
                                    if (spk_len == 1) or\
                                    ((spk_len == 2) and (np.abs(value_diffs[t - spk_len + 1]) < critical_values[int(time_diffs[t - spk_len + 1]) -1, int(month_locs[t])] / 2.)) or \
                                    ((spk_len == 3) and (np.abs(value_diffs[t - spk_len + 1]) < critical_values[int(time_diffs[t - spk_len + 1]) -1, int(month_locs[t])] / 2.) and\
                                      (np.abs(value_diffs[t - spk_len + 2]) < critical_values[int(time_diffs[t - spk_len + 2]) -1, int(month_locs[t])] / 2.)):
                                    
                                        # check if following value is valid
                                        if (value_diffs[t] != st_var.mdi) and (critical_values[int(time_diffs[t]) - 1, int(month_locs[t])] != st_var.mdi) and\
                                            (value_diffs[t] != st_var.fdi):
                                            
                                            # and if at least critical value                                            
                                            if (np.abs(value_diffs[t]) >= critical_values[int(time_diffs[t]) - 1, int(month_locs[t])]):
                                                
                                                # test if surrounding differences below 1/2 critical value
                                                if (np.abs(value_diffs[t - spk_len - 1]) <= critical_values[int(time_diffs[t - spk_len - 1]) - 1, int(month_locs[t])] / 2.): 
                                                    if (np.abs(value_diffs[t + 1]) <= critical_values[int(time_diffs[t + 1]) - 1, int(month_locs[t])] / 2.): 
