...
def dcc_quartile_check(day):
    ...
    for q in range(4):
        if len(day[int(quart_start[q]):int(quart_end[q])].compressed()) > 0:
...
#                            best_estimate_counter[np.argmin(diffs)] += 1
        ...
        for h in range(6):
            if best_fits[h] != -9:
                ...
                if (centre[0] - h + 1) >= 0:
                    if (centre[0] + h + 1 ) <=23:
                        hour_matches[int(centre[0]) - (h + 1) : int(centre[0]) + h + 2] += 1
                    else:
                        hour_matches[int(centre[0]) - (h + 1) : ] += 1
                        hour_matches[ : int(centre[0]) + h + 2- 24] += 1                                        
                else:
                    hour_matches[: int(centre[0]) + h + 2] += 1
                    hour_matches[int(centre[0]) - (h + 1) :] += 1
