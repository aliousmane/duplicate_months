...
def dgc_monthly(station, variable, flags, start, end, plots=False, diagnostics=False, idl = False):
    ...
    for m, month in enumerate(month_ranges):
        ...
        data = st_var.data[int(month[0]):int(month[1])]
        
        filtered = all_filtered[int(month[0]):int(month[1])]
    ...
    if len(good_months[0]) >= MONTH_LIMIT:
        ...
        if len(large_offsets[0]) > 0:
            ...
            for lo in large_offsets[0]:
                flags[int(month_ranges[lo,0]):int(month_ranges[lo,1])] = 1
        ...
        iter = 1
        while good:
            if standardised_months[good_months][(sort_order)][int(mid_point) - iter] != standardised_months[(good_months)][(sort_order)][int(mid_point) + iter]:
                # using IDL notation
                tempvals = [np.abs(standardised_months[(good_months)][(sort_order)][int(mid_point) - iter]),np.abs(standardised_months[(good_months)][(sort_order)][int(mid_point) + iter])]
                ...
                if min(tempvals) != 0:
                    if max(tempvals)/min(tempvals) >= 2. and min(tempvals) >= 1.5:
                        ...
                        if tempvals[0] == max(tempvals):
                            # LHS
                            bad = good_months[0][(sort_order)][:int(mid_point) - iter]
                            if plots: badplot = standardised_months[(good_months)][(sort_order)][:int(mid_point) - iter]
                        ...
                        for b in bad:
                            flags[int(month_ranges[b,0]):int(month_ranges[b,1])] = 1
    ...
    for month in range(12):
        ...
        if windspeeds == True:
            ...
            for y, year in enumerate(month_ranges[:,month,:]):
                ...
                if y == 0:
                    windspeeds_month = np.ma.array(st_var_wind.data[int(year[0]):int(year[1])])
                else:
                    windspeeds_month = np.ma.concatenate([windspeeds_month, st_var_wind.data[int(year[0]):int(year[1])]])
        ...
        if len(this_month_filtered.compressed()) > OBS_LIMIT:
            ...
            if iqr != 0.0:               
                ...
                if GH:
                    ...
                    l_minimum_threshold = round(plot_bincenters[int(good_values[0])]) - 1
                    u_minimum_threshold = 1 + round(plot_bincenters[int(good_values[-1])])
                ...
                if uppercount > 0:
                    ...
                    if gap_start != 0:
                        ...
                        for y, year in enumerate(month_ranges[:,month,:]):
                            ...
                            this_year_data = np.ma.array(all_filtered[int(year[0]):int(year[1])])
                            this_year_flags = np.array(flags[int(year[0]):int(year[1])])
                            gap_cleaned_locations = np.where(((this_year_data - monthly_median) / iqr) > gap_start)

                            this_year_flags[gap_cleaned_locations] = 1
                            flags[int(year[0]):int(year[1])] = this_year_flags
