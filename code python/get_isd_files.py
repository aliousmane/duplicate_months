...
def GetFileListing(host,location,year = ""):
    ...
    try:
        ftpsite=urllib.request.urlopen(host+location+str(year))
        html_dump=ftpsite.readlines()
        """ftpsite=urllib3.PoolManager()
        r=ftpsite.request('GET',host+location+str(year))
        """
    ...
    if year != "":
        outfile = open("input_files/isd_ftp_list_{}.txt".format(year), "w")
    else:
        outfile = open("input_files/isd_ftp_list_parent.txt", "w")
...
def ExtractFileData(file_list, directory = False):
    ...
    for row,line in enumerate(file_list):
        file_creation_date = datetime.datetime.now()   

        if directory:
            size = line.split("'")[1][:1]
        else:
            size = float(line[38:-39])
        file_name = line[57:-6].strip()
 

        if directory:
            if size == "d":
                data_files.append(DataFile(file_name,file_creation_date,0))
        else:
...
#*********************************************
def DownloadStations(DownloadStationList,ToDownload,year, download_file,verbose = False,):
    """
    ...
    **options allows for verbose setting
    ...
    for df,dfile in enumerate(DownloadStationList):
        ...
        if ToDownload[df]:
            ...
            os.chdir(new_loc)
            download_file=open("download_file.txt","w")
            download_file.close()
            download_file=open("download_file.txt","a")
            download_file.write("/* Connexion au serveur*/"+"\n")
            download_file.write("open ftp.ncdc.noaa.gov"+"\n")
            download_file.write("ftp"+"\n")
            download_file.write("oali@canda.ca"+"\n")
            download_file.write("cd /pub/data/noaa/{}\n".format(str(year)))
            
            download_file.write("get "+dfile.filen+"\n")
            download_file.write("close"+"\n")
            download_file.write("quit"+"\n")
            download_file.close()
            os.system("ftp -s:download_file.txt")  
            
            print (' ftp -host '+HOST[6:]+' -cwd /pub/data/noaa/'+str(year)+'/ -get '+dfile.filen+'='+new_loc + dfile.filen)
            #subprocess.call(["ftp",'-host', HOST[6:], '-cwd', '/pub/data/noaa/'+str(year)+'/', '-get', dfile.filen+'='+new_loc + dfile.filen])
...
def main(STARTYEAR, ENDYEAR, restart_id="", end_id="", GetDirList = False):
    ...
    folder_list_file = open("input_files/isd_ftp_list_parent.txt", "r")
    ...
    for year in range(STARTYEAR,ENDYEAR+1):
        ...
        dir_list_file = open("input_files/isd_ftp_list_{}.txt".format(year), "r")
        ...
        DownloadStations(FileGetList,FileFilter,year,download_file, verbose=IsVerbose)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--start', dest='STARTYEAR', action='store', default = 2016,
