...
from qc_tests.duplicate_months import *
import qc_tests.clean_up
import qc_tests.climatological
import qc_tests.clouds
import qc_tests.distributional_gap
import qc_tests.diurnal_cycle
import qc_tests.frequent_values
import qc_tests.humidity
import qc_tests.odd_cluster
import qc_tests.records
import qc_tests.spike
import qc_tests.streaks
import qc_tests.variance
import qc_tests.winds
...
def internal_checks(station_info, restart_id = "", end_id = "", second = False, 
    ...
    qc_code_version = "qc_code_version"#subprocess.check_output(["svnversion"]).strip()
    ...
    for st,stat in enumerate(station_info):     
        ...
        if plots or diagnostics:
            ...
            if first:
                logfile = open(LOG_OUTFILE_LOCS+stat[0]+'.log','w')
            elif second:
                logfile = open(LOG_OUTFILE_LOCS+stat[0]+'.log','a') # append to file if second iteration.
        ...
        if diagnostics or plots: input("stop")
        ...
        if first:
            ...
            # gzip the raw file
            #print(os.path.exists(os.path.join(NETCDF_DATA_LOCS, station.id + ".nc")))
           # subprocess.call(['gzip',NETCDF_DATA_LOCS+station.id + ".nc"])
