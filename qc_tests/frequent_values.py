        ...
        for season in range(5): # Year,MAM,JJA,SON,JF+D
            ...
            if season == 0:
                ...
                for y,year in enumerate(month_ranges_years):
                    ...
                    if season == 1:
                        #mam
                        season_data = np.ma.concatenate([season_data, np.ma.masked_values(filtered_data[int(year[2][0]):int(year[4][-1])], st_var.fdi)])
                    elif season == 2:
                        #jja
                        season_data = np.ma.concatenate([season_data, np.ma.masked_values(filtered_data[int(year[5][0]):int(year[7][-1])], st_var.fdi)])
                    elif season == 3:
                        #son
                        season_data = np.ma.concatenate([season_data, np.ma.masked_values(filtered_data[int(year[8][0]):int(year[10][-1])], st_var.fdi)])
                    elif season == 4:
                        #d+jf
                        season_data = np.ma.concatenate([season_data, np.ma.masked_values(filtered_data[int(year[0][0]):int(year[1][-1])], st_var.fdi)])
                        season_data = np.ma.concatenate([season_data, np.ma.masked_values(filtered_data[int(year[-1][0]):int(year[-1][-1])], st_var.fdi)])
            ...
            if len(season_data) > MIN_DATA_REQUIRED:    
                ...
                for y,year in enumerate(month_ranges_years):
                    ...
                    if season == 0:
                        # year
                        year_data = np.ma.masked_values(st_var.data[int(year[0][0]):int(year[-1][-1])], st_var.fdi)
                        year_flags = station.qc_flags[int(year[0][0]):int(year[-1][-1]),flag_col[v]]
                    elif season == 1:
                        #mam
                        year_data = np.ma.masked_values(st_var.data[int(year[2][0]):int(year[4][-1])], st_var.fdi)
                        year_flags = station.qc_flags[int(year[2][0]):int(year[4][-1]),flag_col[v]]
                    elif season == 2:
                        #jja
                        year_data = np.ma.masked_values(st_var.data[int(year[5][0]):int(year[7][-1])], st_var.fdi)
                        year_flags = station.qc_flags[int(year[5][0]):int(year[7][-1]),flag_col[v]]
                    elif season == 3:
                        #son
                        year_data = np.ma.masked_values(st_var.data[int(year[8][0]):int(year[10][-1])], st_var.fdi)
                        year_flags = station.qc_flags[int(year[8][0]):int(year[10][-1]),flag_col[v]]
                    elif season == 4:
                        #d+jf
                        year_data = np.ma.concatenate([np.ma.masked_values(st_var.data[int(year[0][0]):int(year[1][-1])], st_var.fdi),\
                                                       np.ma.masked_values(st_var.data[int(year[-1][0]):int(year[-1][-1])], st_var.fdi)])
                        year_flags = np.append(station.qc_flags[int(year[0][0]):int(year[1][-1]),flag_col[v]],station.qc_flags[int(year[-1][0]):int(year[-1][-1]),flag_col[v]])
                    ...
                    if season == 0:
                        station.qc_flags[int(year[0][0]):int(year[-1][-1]), flag_col[v]] = year_flags   
                    elif season == 1:
                        station.qc_flags[int(year[2][0]):int(year[4][-1]), flag_col[v]] = year_flags   
                    elif season == 2:
                        station.qc_flags[int(year[5][0]):int(year[7][-1]), flag_col[v]] = year_flags   
                    elif season == 3:
                        station.qc_flags[int(year[8][0]):int(year[10][-1]), flag_col[v]] = year_flags   
                    elif season == 4:
                        split = len(station.qc_flags[int(year[0][0]):int(year[1][-1]), flag_col[v]])
                        station.qc_flags[int(year[0][0]):int(year[1][-1]), flag_col[v]] = year_flags[:split]
                        station.qc_flags[int(year[-1][0]):int(year[-1][-1]), flag_col[v]] = year_flags[split:]
